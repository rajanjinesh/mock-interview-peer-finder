const { rankPeerProfiles, computePeerMatchScore } = require('./src/lib/matchingSystem');
...
async function runMatchingSystemTests() {
  ...
  // --- Test 1: Strong matching peers (Product Manager + Product Design) ---
  ...
  const res1 = rankPeerProfiles(req1, dbPeers);
  ...
  const res2 = rankPeerProfiles(req2, dbPeers);
  ...
  const res3 = rankPeerProfiles(req3, dbPeers);
  ...
  const res4 = rankPeerProfiles(req4, dbPeers);
  ...
  const res5 = rankPeerProfiles(req5, dbPeers);
  ...
  const runA = rankPeerProfiles(req1, dbPeers);
  const runB = rankPeerProfiles(req1, dbPeers);
